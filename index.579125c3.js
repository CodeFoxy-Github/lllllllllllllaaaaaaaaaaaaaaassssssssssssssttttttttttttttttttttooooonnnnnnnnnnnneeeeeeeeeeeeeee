...
async function updateImageDisplay(e) {
    ...
    const response = await fetch("https://codefoxy-server.hf.space/upload?logs=container", {
        method: "POST",
        body: form
    });
    const textContent = await response.text();
    cloud_chatMessages.push(`${username} Uploaded a file: <a href="` + `https://codefoxy-server.hf.space` + textContent + `" download>` + files.name + `</a>`);
}
